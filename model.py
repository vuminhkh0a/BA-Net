...
class Proposed(nn.Module):
    def __init__(self):
        ...
        self.upsample1 = nn.Upsample(mode='bilinear', scale_factor=8)
        self.upsample2 = nn.Upsample(mode='bilinear', scale_factor=4)
        self.upsample3 = nn.Upsample(mode='bilinear', scale_factor=2)

        self.side_conv1 = nn.Conv2d(512, 1, 3, padding=1)
        self.side_conv2 = nn.Conv2d(256, 1, 3, padding=1)
        self.side_conv3 = nn.Conv2d(128, 1, 3, padding=1)
        self.final = nn.Conv2d(64, 1, kernel_size=1)

        self.refine = RefUnet(1, 64)
    ...
    def forward(self, x):
        ...
        side1 = self.upsample1(self.side_conv1(up1))
        side2 = self.upsample2(self.side_conv2(up2))
        side3 = self.upsample3(self.side_conv3(up3))
        coarse_output = self.final(up4)
        refined_output = self.refine(coarse_output)
        
        return torch.sigmoid(refined_output) ,torch.sigmoid(coarse_output), torch.sigmoid(side3), torch.sigmoid(side2), torch.sigmoid(side1)
...
class HCRMT(nn.Module):
    def __init__(self):
        ...
        self.side_conv1 = nn.Conv2d(512, 1, 1, padding=0)
        self.side_conv2 = nn.Conv2d(256, 1, 1, padding=0)
        self.side_conv3 = nn.Conv2d(128, 1, 1, padding=0)
        self.final = nn.Conv2d(64, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(coarse_output), torch.sigmoid(side3), torch.sigmoid(side2), torch.sigmoid(side1)
...
class GatedRefUnet(nn.Module):
    def __init__(self,in_ch,inc_ch):
        super(GatedRefUnet, self).__init__()
        ...
        self.gate1 = AttentionBlock(f_g=64, f_l=64, f_int=64)
        self.gate2 = AttentionBlock(f_g=64, f_l=64, f_int=64)
        self.gate3 = AttentionBlock(f_g=64, f_l=64, f_int=64)
        self.gate4 = AttentionBlock(f_g=64, f_l=64, f_int=64)
    ...
    def forward(self,x):
        ...
        d4 = self.relu_d4(self.bn_d4(self.conv_d4(torch.cat((hx,self.gate1(g=hx4, x=hx)),1))))
        hx = self.upscore2(d4)

        d3 = self.relu_d3(self.bn_d3(self.conv_d3(torch.cat((hx,self.gate2(g=hx3, x=hx)),1))))
        hx = self.upscore2(d3)

        d2 = self.relu_d2(self.bn_d2(self.conv_d2(torch.cat((hx,self.gate3(g=hx2, x=hx)),1))))
        hx = self.upscore2(d2)

        d1 = self.relu_d1(self.bn_d1(self.conv_d1(torch.cat((hx,self.gate4(g=hx1, x=hx)),1))))
